...
#'
#' @param object An output object from clpca
...
bootstrap.clpca = function(object, Bsamples = 1000, myseed = 1){
  ...
  if(is.null(object$X)){
    ...
    for(b in 1:Bsamples){
      ...
      b.out = clpca(Y = Y[obs, ], S = S, start = bstart)
  ...
  if(!is.null(object$X)){
    ...
    for(b in 1:Bsamples){
      ...
      f[, b] = obs
      b.out = clpca(Y = Y[obs, ], X = X[obs, ], S = S, start = bstart)

      # out of sample prediction - quality measure is average deviance
      Yhat = predict.clpca(b.out, X[-obs, ])$Yhat
      devr = rep(NA, R)
      for(r in 1:R){
          G = class.ind(Y[-obs , r])
          YY = Yhat[[r]]
