...
#'
#' @param object An output object from lpca
...
bootstrap.lpca = function(object, Bsamples = 1000, myseed = 1){
  ...
  if(is.null(object$X)){
    ...
    for(b in 1:Bsamples){
      ...
      obs <- f[ , b]
      b.out = lpca(Y = Y[obs, ], S = S, start = bstart)
  ...
  if(!is.null(object$X)){
    ...
    for(b in 1:Bsamples){
      cat("This is analysis", b, "from a total of", Bsamples, "Bootstraps", "\n")
      obs <- f[ , b]
      b.out = lpca(Y = Y[obs, ], X = X[obs, ], S = S, start = bstart)

      # out of sample prediction - quality measure is average deviance
      theta = predict.lpca(b.out, X[-obs, ])$theta
