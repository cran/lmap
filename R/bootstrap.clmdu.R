...
#'
#' @param object An output object from clmdu
...
bootstrap.clmdu = function(object, Bsamples = 1000, myseed = 1){
  ...
  if(is.null(object$X)){
    ...
    for(b in 1:Bsamples){
      ...
      # check for enough levels in each response (> 2)
      ok = TRUE
      while(ok){
        u = rep(NA, ncol(Y)); for(r in 1:R){u[r] = length(unique(Y[obs, r]))}
        if(all(u > 2)){
          ok = FALSE
        }
        else{
          obs = sample(N, N, replace = TRUE)
        }
      }
      f[, b] = obs

      # analyze bootstrap sample
      b.out = clmdu(Y = Y[obs, ], S = S, start = bstart)
  ...
  if(!is.null(object$X)){
    ...
    for(b in 1:Bsamples){
      ...
      b.out = clmdu(Y = Y[obs, ], X = X[obs, ], S = S, start = bstart)

      # out of sample prediction - quality measure is average deviance
      Yhat = predict.clmdu(b.out, X[-obs, ])$Yhat
      devr = rep(NA, R)
      for(r in 1:R){
        G = class.ind(Y[-obs , r])
        YY = Yhat[[r]]
        devr[r] = -2 * mean( log(YY[G == 1] ) )
      }
      sdev.oos[b] <- mean(devr)
